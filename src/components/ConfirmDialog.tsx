...
export function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={onCancel}>
      <div className="bg-bg-secondary border border-border rounded-xl w-[420px] p-6" onClick={e => e.stopPropagation()}>
        <h3 className="text-base font-semibold mb-3">{title}</h3>
        <p className="text-sm text-text-secondary mb-4 leading-relaxed">{message}</p>
        {children}
        {showFeedback && (
          <div className="mb-4">
